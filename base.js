...
/* Anchor first character, only return that 1 character*/
String.prototype.anchor = function StringAnchor(){
  return this.mod(this.isAnchor() ? 0 : 1<<8);
}
...
String.prototype.deAnchor = function StringAnchor(){
  return this.mod( this.isAnchor() ? -1<<8 : 0);
}
/*Anchor an entire possibly encoded string*/
String.prototype.anchorify = function ScreenLinkify(){
  let combo = this.decode();
  combo.color.each( (c,i) => combo.color.set(i, (c || " ").anchor()));
  return String.encode(combo);
}
